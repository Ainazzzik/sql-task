...
select distinct first_name, length (first_name) as length,h.* from owners join houses h on owners.id = h.owner_id order by length desc limit 1;
-- 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
select true from houses where price between 1500 and 2000 limit 1;
-- 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select * from addresses join houses h on addresses.id = h.address_id where addresses.id in (5,6,7,8,10);
-- 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select h.*,h.furniture, o.first_name, c.first_name,name from houses h
                                                                 join rent_info ri on h.id = ri.customer_id
...
select * from customers where date_of_birth between date '1999/1/1' and now() limit 25 offset 10;
-- 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
select house_type,rating,first_name from houses join owners o on o.id = houses.owner_id order by rating;
--desc
select house_type,rating,first_name from houses join owners o on o.id = houses.owner_id order by rating desc;
...
select house_type,furniture,o.first_name,city,street from rent_info
                                                              join houses h on h.id = rent_info.house_id
...
select * from customers join rent_info ri on customers.id = ri.customer_id
...
select * from houses join rent_info ri on houses.id = ri.house_id
                     join customers c on ri.customer_id = c.id;
...
select o.first_name, c.first_name from rent_info
                                           join houses h on h.id = rent_info.house_id
                                           join owners o on o.id = rent_info.owner_id
                                           join customers c on c.id = rent_info.customer_id
where o.first_name ilike 'A%';

-- 16.Эн уйу коп owner_ди жана анын уйлорунун санын чыгар.
select o.first_name , count(h.id) from rent_info
                                           join owners o on o.id = rent_info.owner_id
...
select house_type, room, city , street , first_name from rent_info
                                                             join houses h on rent_info.house_id = h.id
                                                             join addresses a on h.address_id = a.id
                                                             join owners o on h.owner_id = o.id
group by house_type, room, city, street, first_name order by max(room) desc limit 1;
...
select first_name,price from rent_info
